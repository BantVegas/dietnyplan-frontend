  ...
  return (
    <footer className="w-full bg-white/60 backdrop-blur-md border-t border-white/30 py-6 px-4 sm:px-8 mt-auto flex flex-col sm:flex-row items-center justify-between gap-4 text-center">
      ...
      <div className="flex flex-wrap gap-6 justify-center text-gray-600 text-sm">
        ...
        <a href="#kontakt" className="hover:text-teal-700 transition">
          Kontakt
        </a>
        <a href="#podmienky" className="hover:text-teal-700 transition">
          Obchodné podmienky
        </a>
        <a
...
// ---------------------- VEKOVÁ VOĽBA ŽENY ----------------------
const ageOptions = [
  { label: "18–26", img: "/images/zena18-26.png" },
  { label: "27–38", img: "/images/zena27-38.png" },
  { label: "39–50", img: "/images/zena39-50.png" },
  { label: "51+", img: "/images/zena51+.png" },
];
...
export default function SelectAgeWoman() {
  ...
  useEffect(() => {
    document.title = "Výber veku ženy | dietnyplan.sk";
    ...
    meta.setAttribute(
      "content",
      "Zadaj svoj vek a získaj diétny plán šitý na mieru podľa potrieb žien v tvojom veku. Každá životná etapa ženy má svoje špecifiká a zaslúži si individuálny prístup."
    );
  }, []);
    ...
    <div className="min-h-screen flex flex-col bg-gradient-to-tr from-teal-100 via-white to-teal-200">
      ...
      <main className="flex-1 flex flex-col items-center pt-28 px-2">
        ...
        <p className="max-w-xl text-center text-gray-700 text-base sm:text-lg mb-8">
          Každá životná etapa ženy má svoje špecifiká – od metabolizmu, hormonálnych zmien až po životné tempo.
          <br />
          Výber správneho veku nám umožní prispôsobiť{" "}
          <b>diétny plán presne tvojim potrebám</b>, nech už si vo fáze kariéry, materstva alebo menopauzy.
          <br />
          Naša starostlivosť je <span className="text-teal-600 font-bold">individuálna</span> a rešpektuje rozdiely v každom veku.
        </p>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 w-full max-w-5xl">
          {ageOptions.map(({ label, img }) => (
            ...
            >
              <img
                src={img}
                alt={`Veková skupina ${label} rokov`}
